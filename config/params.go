...
var ChainParams = Params{
	...
	// BIP 173.
	Bech32HRPSegwit: "bt", // always ms for main net
...
// auto calc genesis block head sign(2020-06-19 danny)
func autoCalcGenesisBlockHeadSign() {
	//new PrivKey
	priv, err1 := pocec.NewPrivateKey(pocec.S256())
	if err1 != nil {
	...
	//setting pub key
	genesisHeader.PubKey = pub

	//calc hash
	pocHash, _ := genesisHeader.PoCHash()
	dataHash := wire.HashH(pocHash[:])

	//calc sign
	sig, _ := priv.Sign(dataHash[:])
	genesisHeader.Signature = sig
}

// Validate Genesis Block Head Sign(2020-06-19 danny)
func validateGenesisBlockHeadSign() bool {
	pocHash, _ := genesisHeader.PoCHash()
	dataHash := wire.HashH(pocHash[:])

	flag := genesisHeader.Signature.Verify(dataHash[:], genesisHeader.PubKey)
...
func init() {
	...
	// auto calc genesis blockHead pubkey and sign value(2020-06-19 danny)
	autoCalcGenesisBlockHeadSign()
	// check genesis blockHead sign validation(2020-06-19 danny)
	flag := validateGenesisBlockHeadSign()
	if !flag {
		logging.CPrint(logging.ERROR, fmt.Sprintf("validate genesis blockhead sign error!!!!"))
	}
	// register chainParams
	var regErr = Register(&ChainParams)
	if regErr != nil {
		panic(regErr) // should not happen
	}
}
